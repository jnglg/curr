...
@title{Leçon 2: Contrats, chaînes et images}
...
@unit-lessons{
@lesson/studteach[
     #:title "Révisions sur les cercles d'évaluation"
     #:duration "30 minutes"
     ...
     #:preparation @itemlist[@item{OPTIONAL: Hand out @(hyperlink "https://docs.google.com/document/d/1Qn59Fol2tspqOx6XQV88xm-IYsRGY769cb7MQeknSMA/edit?usp=sharing" "Warmup activity sheet").}
                              @item{Les cahiers des élèves et de quoi écrire.}]
     ...
     #:pacings (list 
        ...
        @points[@point{@student{@activity[#:forevidence (list "BS-CE&1&2" "BS-CE&1&4" "A-SSE.1-2&1&3")]{
                       ...
                       @itemlist[#:style 'compact
	                         @item{Sur chaque ligne, il y a une expression mathématique écrite dans la colonne de gauche.}
                                 @item{Pour chacune, dessine le cercle d'évaluation correspondant dans la colonne du milieu.}
                                 @item{Quand elles sont toutes faites, convertis chaque cercle d'évaluation en un programme que tu peux entrer dans l'ordinateur. Fais attention aux parenthèses et laisse bien un espace entre les données.}]}}
...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[          
            @point{@student{Les cercles d’évaluation sont un outil très puissant et on peut les utiliser pour manipuler bien plus que de simples nombres.
                            Considérons l’exemple suivant : @sexp{(star 50 "solid" "red")}
                            @activity[#:forevidence "N-Q&1&2"]{@itemlist[@item{Quel est le nom de la fonction utilisée ?}
                                                 @item{Combien de paramètres sont donnés à cette fonction ?}
                                                 @item{À votre avis, que peut faire cette fonction ?}]}
             ...
             @point{@student{Les règles que nous avons vues pour transformer un cercle d’évaluation en code Racket n’ont pas changé. 
                             Par exemple, pour le cercle ci-dessus, cela donne : @sexp[#:form "code"]{(star 50 "solid" "red")}
                             @activity[#:forevidence "BS-PL.1&1&2"]{Tape ce code dans la fenêtre des interactions et appuie sur "Entrée". Qu’as-tu obtenu ?
                                                                    @itemlist[@item{Que fait la fonction @code{star} ?}
                                                                               @item{Recommence mais avec un nombre plus grand que @code{50}. À quoi sert le premier paramètre de la fonction ?}
                                                                               @item{Recommence mais avec @code{"outline"} à la place de @code{"solid"} (n’oublie pas les guillemets !)  À quoi sert donc le deuxième paramètre ?}
                                                                               @item{Maintenant, remplace @code{"red"}  par quelque chose d’autre… À toi de jouer ! (Là non plus, n'oublie pas les guillemets !) À quoi sert ce troisième paramètre ?}]
             ...
             @point{@student{Dans ces expressions, nous avons utilisé un nouveau type de valeur possible. @code{"Solid"} et @code{"red"} sont des exemples d’un nouveau @vocab{type de donnée} appelé @vocab{chaîne de caractères}, simplement @vocab{chaîne}, ou en anglais @vocab{String}.  
                             @bannerline{Une chaîne ou String est simplement quelque chose d’écrit entre guillemets. Nous utiliserons souvent le mot String car c’est celui utilisé par Racket.} 
                             }
